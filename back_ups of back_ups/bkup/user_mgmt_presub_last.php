...
function read_id($id_num){
	...
	*/
	...
	//~ echo "file: $file_name line #: ".$MAX_ULINE_LEN*($id_num%$USERS_PER_FILE)."<br><br>";
	$user_obj= new User;
	$user_obj=unflush_user($user_base);
	//~ print_r($user_obj);
	fclose($user_base);
...
function write_woot($home_user, $woot, $timestamp){
	...
	//~ echo "$err_no c<br>"; $err_no++;

		for($index=0; $index<$WOOTS_PER_LINE; $index++){
	//~ echo "$err_no d<br>"; $err_no++;

			$read_woot=trim(fread($woot_file, $MAX_WOOT_LEN+1+$MAX_WOOT_TMSTP+1), $MY_DELIMITER );
			// echo "read woot: $read_woot<br><br>";
			if(empty($read_woot)) {
	//~ echo "$err_no e<br>"; $err_no++;

				// echo "found an empty woot<br><br>";
...
function load_woots($home_user, $num_woots){
	...
	$row_val=(int)($home_user->id/$USERS_PER_FILE);
	$col_val=(int)(($home_user->num_woots -1 )/$WOOTS_PER_LINE); // if have 20 woots, only in column 1 still

	$file_name="woots/w_r".$row_val."c".$col_val.".txt";
		echo $file_name." HERE2<br> num woots: ".$home_user->num_woots." $woots_read read and $num_woots to read<br>";
if (file_exists($file_name))
{
	echo "YEP<br>";
}else { echo "NOPE<br>"; }
	...
	$woots_read=0;
	$woots=array();
		//~ echo "row val $row_val<br>in here<br>num $num_woots<br>fn $file_name<br>";
	while( file_exists($file_name) && $woots_read < $num_woots){
				echo "entering new file: $col_val of name $file_name<br>";

		// read woot by woot for empty spot, and write it and return
		//~ echo "opened file: $file_name as ".$home_user->username." with $woots_read read<br>";
		$woot_file=fopen($file_name, "r+") or die("Could not open file: ".$file_name);
		fseek($woot_file, (1+$home_user->id%$USERS_PER_FILE)*(1+$MAX_WOOT_LINE_LEN));
		// go to user's line there
				echo "entering new file: $col_val<br>";
		for($index=0; $index<$WOOTS_PER_LINE && $woots_read < $num_woots; $index++){
			...
			//~ fseek($woot_file, -($MAX_WOOT_TMSTP));
			
//			fseek($woot_file, -(1+$MAX_WOOT_LEN), SEEK_CUR);
			
			//~ $read_woot->message=trim(fread($woot_file, ($MAX_WOOT_LEN+1)), $MY_DELIMITER );

			echo "got this far: ".$read_woot->message." and: ".$read_woot->timestamp."<br>";
			 //~ echo "read woot: $read_woot<br><br>";
			if(!empty($read_woot->message)) {
				//~ echo "found a non empty woot: $read_woot->message <br><br>";
				//~ echo "not empty woot? : $read_woot->message <br>";
				$woots_read++;
				array_push($woots, $read_woot);				
			} // if a woot exits, just read the next woot
			//~ fseek($woot_file, -($MAX_WOOT_LEN+1), SEEK_CUR);
			//~ fseek($woot_file, -(1+1+$MAX_WOOT_LINE_LEN), SEEK_CUR);
		}
		fclose($woot_file);
		$col_val--;
		$file_name="woots/w_r".$row_val."c".$col_val.".txt";
		//~ echo $file_name." HERE + read $woots_read<br>";
	}
	return $woots;
}
