					...
					if(isset($_POST['login_sub'])){
						...
						if(isset($user_obj->username)){
							if( $user_obj->password != $_POST['password'] ) {
								...
								if($user_obj->free_bit=="d"){ // if you were deactivated, by logging in you are now activated 
									$user_obj->free_bit="n"; 
									$u_file="users/u_".(int)($user_obj->id/$USERS_PER_FILE).".txt";
									$user_base=fopen($u_file, "r+") or die("Could not open file: ".$u_file);
									flush_user($user_base, $user_obj, ($user_obj->id%$USERS_PER_FILE)*$MAX_ULINE_LEN);
									fclose($user_base);
								}
