...
function follow($home_user, $other_user){
	...
	*/
	...
	if(!isset($flr_num)){
		...
		// fseek($following, ($lnm*(MAX_FLR_LINE_LEN)) + ($MAX_ID_LEN*$fle_num));
	...
	if(!isset($fle_num)){
		...
		fwrite($following, str_pad($follower_id, $MAX_ID_LEN, "."));
		// fwrite($following, str_repeat($MY_DELIMITER, $MAX_ID_LEN));
		fclose($following);
