...
/*
 1. All file paths hardcoded to /var/www/html. Consider fixing this so its the directory code runs in (same call as OS, get..?)
 2. register.php filters out spaces in first/last_name, POSSIBLE to let that be by storing them as _ and when returning an unflushed user replacing _ with ' ' 
	except leading/trailing spaces, cut those out
ACTUALLY: just don't be lazy, use actual reads instead of >>, have read code and tester code already
 3. Make message size not 560 but a global both here and in php
 3b. adjust woot marshalling accordingly on both ends ??
 4. Replace exit() with an exit_net(errmsg)? it shouldnt crash the program
 * 		log errors to an error log and return to the next connection
5. Check load_following code to see how it handles a # greater than # of followers
* 			note somewhere that since 560 bytes a limit to # to followers to send over
* 			need an explode_flwes pretty much identical to explode_woots
6. reply(connfd, user_explode(flush)) is unnecessary traffic, get rid of it, no good reason
* 	flush() doesnt even take a user back on the php end
7. test load_woots_seq
8. What to do if invalid commands passed? just next connection? say something?
*  		have every php_net function check for ERROR field as first arg, and echo from there?
9. Oddly, must be logged in to call seeall.php

10. Document each function at header of function defn

11. make a copy of this file, then remove comments/output testing for clean submit
*  */

// peek actually moves the pointer
// MUST USE CLEAR() once hit end of file with seekg, cant evne move seekp until clear
// crashes are only for impossible-state scenarios. if initial set up registers at least 1 user, everything taken care of okay
// make a note for each function that returns heap memory about caller responsibility
// returns a nullptr if user couldnt be found for unflush, read uer, read id
...
void reply_woots(int connfd, vector<string>* woots){
		/*
		reply_woots(connfd, vector<string>*) takes in a pointer to a vector of strings, representing a list of
		* woot messages belonging to some one user, and proceeds to send those strings out over the network
		* four at a time. Since the default message size is 560 bytes, and each woot is about 130 bytes,
		* only 4 are sent a time.
		*/
	...
	void handle_php(int connfd, char* cmd, int cmd_size){
		...
		*/
		...
		if(received_str=="create_new_user"){
			...
			// a real practical limit of 560 bytes to load_following at one time
			User* home_user=user_unexplode(received_ss);
			string group_type;
    ...
    // We had a connection.  Do whatever our task is.
		...
		if(read_well==sizeof(cmd)){ handle_php(connfd, cmd, 560); }
    // else fails silently
...
User* unflush_user(string file_name, int offset, bool unsure){	
	...
	*/
	...
	// constant a new user object to pull out of the file database
