...
// check for any empty fields once not html code
				...
				else if(preg_match('/\s/',$_POST['first_name']) ) { echo "<p>Please re-enter your first name without any spaces.</p>"; }
				if( empty($_POST['last_name']) ) { echo "<p>Please fill in your last name.</p>"; }
				else if(preg_match('/\s/',$_POST['last_name']) ) { echo "<p>Please re-enter your last name without any spaces.</p>"; }
				...
				if( empty($_POST["first_name"])|| empty($_POST["last_name"]) || empty($age) || empty($email) || empty($username) || empty($password) || empty($re_password) || preg_match('/\s/',$_POST['first_name']) || preg_match('/\s/',$_POST['first_name']) ){
...
// if no errors, go to login.php
				if(!any_errors($full_name, $age, $email, $username, $password, $re_password)){
					...
					if(!isset($user_obj->id)){
						...
						// had to add because of network code
						$user_obj->id=-1; // will be set later
